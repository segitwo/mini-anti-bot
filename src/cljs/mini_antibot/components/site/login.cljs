(ns mini-antibot.components.site.login)
        ...
        [:div {:class "w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700"}
         [:div {:class "p-6 space-y-4 md:space-y-6 sm:p-8"}
          ...
          [:form {:class "space-y-4 md:space-y-6"}
            ...
            [:button {:type "button" :class "w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"} "Войти"]
            [:p {:class "text-sm font-light text-gray-500 dark:text-gray-400"} "Еще нет акаунат зарегистрируйтесь?"
             [:a {:href "#" :class "ml-1 font-medium text-primary-600 hover:underline dark:text-primary-500"} "Зарегистрироваться"]]]]]])
