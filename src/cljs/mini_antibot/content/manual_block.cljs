(ns mini-antibot.content.manual-block
  (:require
   [reagent.core :as r]
   [mini-antibot.api-client :refer [api-client-error add-ip]]))
...
(defn manual-block []
  [:<>
   [:h3 {:class "mb-1 font-medium"} "Блокировать по IP"]
   [:div {:class "flex h-24 mb-4 rounded bg-gray-50 dark:bg-gray-800"}
    [:div
     [:div {:class "flex h-20 mb-4 rounded bg-gray-50 dark:bg-gray-800 items-top mx-5 gap-5 flex flex-row mt-4"}
      [:div {:class "grid gap-6 mb-6 md:grid-cols-2 sm:grid-cols-3 xs:grid-cols-4"}
       [ip-input ip]
       [:div
